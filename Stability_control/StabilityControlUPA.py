...
class Aircraft:
    def __init__(self,                      # class initial state; automatically invoked during class instantiation; arguments are taken from __init__() method and stored in similarly named class attributes
                 ...
                 _hor_surface_aera, _ver_surface_aera, _htail_span, _vtail_span, _root_vtail, _tail_arm, _root_htail, 
                 _inertia, _v_stall, _v_cruise, _OEW_mass,
                 _payload_c, _payload_a, _weight_fuel, _LE_MAC, _arm_fuel_tank,
        ...
        self.tail = {'S_h': _hor_surface_aera, 'S_v': _ver_surface_aera,'b_h': _htail_span, 'b_v':_vtail_span, 'Ç_r_v': _root_vtail ,'arm': _tail_arm, 'Ç_r_h': _root_htail }
        ...
        self.LE_MAC, self.weight_fuel, self.arm_fuel_tank = _LE_MAC, _weight_fuel*grav, _arm_fuel_tank   # self.weight_ = <weight> [N]
    ...
    def getWeightDry(self, aero=0): 
        ...
        weight_dry = payl_weight + self.OEW['OEW_weight']
        weight_dry_moment = payl_moment + self.OEW['OEW_weight']*self.OEW['OEW_cg_loc']
...
class Flight:
    ...
    def loadingdia(self, fuel_used, aero=0):   #fuel > 0: bagage > 1: pas_2 > 2: pas_1 > 3: pilot_2 > 4: pilot_1
        moment_emp, weight_emp = self.aircraft.OEW['OEW_weight']*self.aircraft.OEW['OEW_cg_loc'], self.aircraft.OEW['OEW_weight']
        ...
        cg_LEMAC_ld = np.array([self.aircraft.OEW['OEW_cg_loc'], (moment_emp+moment_fl)/(weight_fl+weight_emp)])
        weight_LEMAC_ld = np.array([self.aircraft.OEW['OEW_weight'],(weight_fl+weight_emp)])
    ...
    def rudderSiz(self, ccr_ratio, init, max_def, wake_fac, eff,CL_av, aero=0):
        ...
        inertia_b = np.matrix([[self.aircraft.inertia['xx']], 
                               [self.aircraft.inertia['zz']], 
                               [self.aircraft.inertia['xz']]])
        inertia_w = inertia_trans_w*inertia_b
        N_SR = ((inertia_w[0,0]*inertia_w[1,0] - (inertia_w[2,0]**2))/inertia_w[0,0])*spin_rate
        b_eR_v = (2*N_SR)/(max_def*init.isa["dens"]*(init.airspeed**2)*CL_av*self.aircraft.tail["arm"]*(1-wake_fac)*self.aircraft.tail['S_v']*eff*eff)
        if b_eR_v < 1.: 
            return {'b_ev/b_v': b_eR_v}  
        else:
    ...
    def Longstabfixed(self, init, cc_ratio, max_def): 
        ...
        CM_delta_elv = (self.aircraft.coeff['tailvel_eff']*(self.aircraft.tail['S_h']/self.aircraft.wing['S'])*((X_cg_flight - X_AC_w)/self.aircraft.wing['MAC']) - V_HT)*self.aircraft.coeff['L_alpha_ht']*elv_rat
        ...
        elv_area = cc_ratio*self.aircraft.tail['S_h']

        if ((X_nfix - (self.aircraft.coeff['SM']*self.aircraft.wing['MAC'])) > self.loadingdia(0)['most_aft_ld']) or ((X_nfix - (self.aircraft.coeff['SM']*self.aircraft.wing['MAC'])) > self.loadingdia(fuel_used)['most_aft_ld']): 
            return True, {'CL_delta_elv': CL_delta_elv, "CM_delta_elv": CM_delta_elv, 'X_nfix': X_nfix, 'Elv_aera': elv_area }
        else: 
            return False
    def space_state_sys(self, init, motion, t_end, rud = 0):      #motion: "shortper", "phugoid", "aperiotic", "dutch", t_end = end time of array
        ...
        Kx = sqrt(self.aircraft.inertia['xx']/((init.weight/grav)*(self.aircraft.wing['b']**2)))
        Kz = sqrt(self.aircraft.inertia['zz']/((init.weight/grav)*(self.aircraft.wing['b']**2)))
        Kxz = self.aircraft.inertia['xz']/((init.weight/grav)*(self.aircraft.wing['b']**2))
        Ky = sqrt(self.aircraft.inertia['yy']/((init.weight/grav)*(self.aircraft.wing['MAC']**2)))
        if sym: 
            C1 = np.matrix([[-2.*mass_coeff_c*(self.aircraft.wing['MAC']/(V**2)), 0 , 0, 0], 
                            ...
                            [0, self.aircraft.m_der_pitch["alpha_dot"]*(self.aircraft.wing['MAC']/V), 0, -2.*mass_coeff_c*(Ky**2.)*((self.aircraft.wing['MAC']/V)**2.)]])
            ...
            C1 = np.matrix([[(self.aircraft.f_der_lat["beta_dot"] - 2*mass_coeff_b)*(self.aircraft.wing["b"]/V), 0, 0, 0], 
                            ...
                            [0, 0, -2*mass_coeff_b*(Kx**2)*((self.aircraft.wing["b"]/V)**2), 2*mass_coeff_b*(Kxz)*((self.aircraft.wing["b"]/V)**2)], 
                            [self.aircraft.m_der_yaw["beta_dot"]*(self.aircraft.wing["b"]/V), 0, 2*mass_coeff_b*(Kxz)*((self.aircraft.wing["b"]/V)**2), -2*mass_coeff_b*(Kz**2)*((self.aircraft.wing["b"]/V)**2)]])
    ...
    def responsePlot(self, init, motion, t_end, indic, rud = 0):
        ...
        #plt.show() 
        mot, eigv_list = ["phugoid","dutch","aperiodic"], []
        for mot_indecc in mot: 
            resp = self.space_state_sys(init, mot_indecc, t_end, rud)
            eigv_list.append(resp[5][0])
        eigv_mot  = {"shortper": eigv_list[0][0], "phugoid":eigv_list[0][2], "dutch": eigv_list[1][0], "aperiodic": [eigv_list[1][3], eigv_list[1][2]]}
        return {"eigenvalue": eigv_mot}
...
OEW_mass = OEW_cg(("wing",  692.86,   7.42), 
                ("engine",  692.86,   7.42), 
...
c550 = Aircraft(30., 2.0569, 15.911, 0.316, 0., 2.15,                 
                ...
                {"xx": 19996.17, "yy": 24491.75, "zz": 44202.064, "xz": 44202.064}, 62, 92.6, OEW_mass,  # class instantiation
                load_c, load_a, 1814.39, inch(261.), inch(300), 
                ...
                4.1, 3.91, 0.0197, 0.45, 0.9, 0., 0.05)
                 ...
                 #_inertia, 65., 
                 #_payload_c, _paload_a ,_weight_bempty, _weight_bempty_loc, _weight_fuel, _LE_MAC, _arm_fuel_tank,
                 #_f_der_long, _f_der_lat, _f_der_norm, _m_der_pitch, _m_der_yaw, _m_der_roll, _payl_loc, _req_rolrate,  
                 #_L_alpha_w, _L_alpha_ht, _m_pitch_alpha_ac, _downwash, _tailvel_eff, _ih, _stab_marg):  
...
#STABCH = run_c.stablecheck(init_cruise_c, 0.5, 0.4363)
...
#rud_size = run_c.rudderSiz(0.5, init_rud_c, 0.349066, 0, 0.96, 5.4 )                      #cca_ratio, init, max_def, wake_fac, tail_eff,CL_av
#loading_dia =  run_c.loadingdia(1814.369, 0)
#response_plot =  run_c.responsePlot (init_cruise_c, "dutch",200, "sslipa", 1)    # init, motion, t_end, indic, rud = 0
