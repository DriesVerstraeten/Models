...
def shear_box(dx,dy,start,end,i,t,rho,fh,rib1,rib2): #shear stress calc. dx dy are grid numbers (use 500 for each or so). start end are %cord where wingbox is (0.1 and 0.575 for wing). i is the location where you calculate shear (so if its 499 for dx=500 its at the root. or tip if you defined it like that). t thickness, rho is rho, fh is the force on the wing
    ...
    xcoordinates1 = xcoordinates * wing.cord[i]
    ycoordinates1 = ycoordinates * wing.cord[i]
    ...
    Ixx = moi.wingbox_MOI()[0] 
    Iyy = moi.wingbox_MOI()[1]
    Ixy = moi.wingbox_MOI()[2]
    y_NA = moi.wingbox_MOI()[5]
    x_NA = moi.wingbox_MOI()[6]
    slope1 = np.zeros(dx)
    slope2 = np.zeros(dx)
    for j in range (0,dx):
    ...
    q4[0] = q3[0]
    for j in range (1,dx):
        q4[j] = q1[j-1] - (fh*(abs(x_y_down[0]-x_y_up[0])/dx)*t*((x_y_left[j]-y_NA)*Iyy[i]-(x_x[0]-x_NA[i])*Ixy[i]))/(Ixx[i]*Iyy[i]-Ixy[i]**2)
    q1[0] = q4[dx-1]
